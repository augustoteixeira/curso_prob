...
Usaremos a notação $A^{\cc}:=\gO\setminus A$ para denotar o evento \emph{complementar} de $A$. \index{evento}
...
\begin{definition}
  Quando $\Omega$ é um espaço topológico (ou em particular, um espaço métrico), temos um exemplo importante dado pela \emph{$\sigma$-álgebra de Borel} \index{sigma-algebra@$\sigma$-álgebra!de borel}, $\mathcal{B}(\Omega)$, gerada pelos abertos da topologia de $\Omega$.
\end{definition}

\medskip

\todo{certeza que queremos a notação abaixo?}
...
\begin{definition}
  Dizemos que uma classe $\mathcal{A} \subseteq \mathcal{P}(\Omega)$ é um $\pi$-sistema \index{pi sistema@$\pi$-sistema} se for fechado por intersecções finitas, isto é: para todos $A, B \in \mathcal{A}$ temos $A \cap B \in \mathcal{A}$.
\end{definition}
...
Bertrand propôs dois métodos para realizar esse cálculo. \index{Paradoxo de Bertrand@Paradoxo de Bertrand}
