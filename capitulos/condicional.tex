...
\section{Teorema da Extensão de Kolmogorov}
...
\begin{proof}
  ...
  Usando a notação $f^{-1}$ para a inversa e $f^{(-1)}$ para a pré-imagem para evitar ambiguidades, temos
  \begin{equation*}
    \begin{split}
      P\big(A_1 \times \dots \times A_n \times
      & E_{n+1} \times \dots\big) = \widebar{P}
        \big(\Psi^{(-1)}(A_1 \times \dots \times
        A_n \times E_{n+1} \times \dots)\big)\\
      & = \widebar{P} \big( \psi^{(-1)}_1(A_1)
        \times \dots \times \psi^{(-1)}_n(A_n)
        \times \mathbb{R} \times \dots \big)\\
      & = \widebar{P}_n (\psi^{(-1)}_1(A_1) \times
        \dots \times \psi^{(-1)}_n(A_n))\\
      \intertext{pela definição de $\widebar{P}_n$,}
      & = P_n \Big( \phi^{(-1)}_1\big(\psi^{(-1)}_1(A_1))
        \times \dots \times \phi^{(-1)}_n
        \big(\psi_n^{(-1)}(A_n)\big) \Big)\\
      \intertext{mas $x \in \phi^{(-1)}(\psi^{(-1)}(A))$
      se e só se $\phi_i(x) \in \psi^{(-1)}(A)$, que é equivalente a
      $\psi(\phi(x)) \in A$ e a $x \in A$, logo}
...
\begin{proof}
  ...
  \item Dado $D \in \mathcal{B}(\mathbb{R})$, $(\phi')^{-1}(D) = A \cap \phi^{-1}(D)$ que é da forma $A\cap C$ com $C\in \cB(\bbR^d)$.
...
\begin{proof}
  ...
  \todo{Mostrar isso usando $B(x, 2^{-j}) = \{(x_1, \dots, x_j, z_{j + 1}, \dots) \}$}
  ...
  \todo{$\phi(n_1, \dots, n_k, m_{k + 1}, \dots)$ está contido em um intervalo exclusivo.}
\end{proof}
...
\begin{proof}
  ...
  Seja $G = \{(w_1, \dots, w_l)\} \times \mathbb{N}^\mathbb{N}$ (esses conjuntos geram a $\sigma$-álgebra canônica em $\mathbb{N}^\mathbb{N}$).
  Se $\phi^{-1}(G) = \varnothing$, não temos nada a provar.
  Se existe $x$ tal que $\phi(x) = w \in G$, $\phi^{-1}(G) = A_{(\phi_1(x), \dots, \phi_l(x))}$, de forma que mostramos que $\phi$ é mensurável.
...
\todo{dar exercícios que mostram a diversidade dessa classe de espaços canônicos: finitos, enumeráveis, superfícies, uniões, interseções, produtos finitos, produtos enumeráveis...}
...
\begin{proof}
  ...
  \begin{enumerate}[\quad a)]
  \item $Y$ é $\mathcal{F}'$-mensurável e
  \item $\mu(A) = \int_A Y \d P$ para todo $A \in \mathcal{F}'$.
  \end{enumerate}
...
Porém ainda falta um ingrediente bastante importante para construir ou verificar que determinadas variáveis são esperanças condicionais: limites de funções.
...
\begin{theorem}[Teorema da Desintegração] \index{Teorema!da Desintegracao@da Desintegração}
  \label{desintegracao}
  Sejam espaços mensuráveis $(\Omega, \mathcal{F})$ e $(S, \mathcal{S})$, com $S$ canônico.
  Se $P$ é uma probabilidade no espaço produto $(\Omega \times S, \mathcal{F} \otimes \mathcal{S})$ e denotamos por $P_1 = P_{X_1}$ a
  distribuição marginal da primeira coordenada, então existe um núcleo de transição $K: \Omega \times \mathcal{S} \to [0,1]$ satisfazendo
  \begin{equation}
    P = P_1 \star K,
  \end{equation}
  Em particular,
  \begin{equation}
    ...
    P(A \times B) = \int_A K(\omega, B) P_1 (\d \omega) \text{ para todo $A \in \mathcal{F}$, $B \in \mathcal{S}$}.
  \end{equation}
...
\begin{proof}
  Como de costume, basta provar o teorema para o caso $(S, \mathcal{S}) = (\mathbb{R}, \mathcal{B}(\mathbb{R}))$.
  De fato, se assumimos a validade do teorema para a reta, podemos usar a função bi-mensurável $\phi: S \to B \in \mathcal{B}(\mathbb{R})$ para concluir o caso geral.

  Nos restringiremos agora ao espaço $(\Omega \times \mathbb{R}, \mathcal{F} \otimes \mathcal{B}(\mathbb{R}), P)$.
  Para cada $q \in \mathbb{Q}$, definimos $P^q_1 : \mathcal{F} \to [0,1]$ por
  \begin{equation}
    P^q_1 (A) = P \big( A  \times (-\infty, q] \big).
  \end{equation}
  Observando que $P^q_1$ é uma medida (sub-probabilidade) absolutamente contínua com respeito a $P_1$, podemos definir
  \begin{equation}
...
\todo{Coisas a melhorar: renomear $\Omega$ para $E$ e $E$ para $F$. Ou substituir $E$ por $F$ no teorema anterior e $\Omega$ por $F$ aqui.}
...
\begin{theorem}
  ...
  Sejam $(S, \mathcal{S}, P)$ e $(E, \mathcal{A})$ espaços mensuráveis canônicos.
  Considere também $X: S \to E$ um elemento aleatório, então existe um núcleo de transição $K$ de $E$ a $S$ tal que
  \begin{equation}
    \label{e:reg_cond_prob_givenX}
    K(X(\omega), F) = E[\1_{F} | X], \text{ para todo $F \in \mathcal{S}$}.
  \end{equation}
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[scale=3]
    \draw[->,gray,very thin] (0, -.8) -- (0,1.2) node[left, black] {$S$};
...
\begin{proof}
  Defina o elemento aleatório $W: S \to E \times S$, dado por $W(\omega) = (X(\omega), \omega)$, que percorre o gráfico de $X$ (representado horizontalmente).
  Observe que a medida $P_W$ possui marginais $(X_1)_* P_W = X_* P$ e $(X_2)_* P_W = P$.
  Como $P_W$ satisfaz as condições do Teorema~\ref{desintegracao}, existe um núcleo $K: E \times \mathcal{S} \to [0,1]$ tal que para todo $A \in \mathcal{A}$, $F \in \mathcal{S}$,
  \begin{equation}
    P_W(A \times F) = \int_A K(x, F) P_X (\d x).
  \end{equation}
  Fixado $F \in \mathcal{S}$, $K(X(\omega), F)$ é obviamente $\sigma(X)$ mensurável, por ser uma composição de uma função mensurável em $E$ com $X$.
  ...
  \begin{lemma}
    Se $X : S \to E$ é um elemento aleatódio tomando valores em um espaço $E$ canônico, então seu gráfico $G = \{(X(\omega), \omega); \omega \in S\}$ é mensurável na $\sigma$-álgebra produto $\mathcal{A} \otimes \mathcal{F}$.
  \end{lemma}
  ...
  Retornando à prova de \eqref{e:princ_substit}, já sabemos que $G' = \{(X(\omega), \omega); \omega \in S\}$ é mensurável.
  Além disso, por definição $P_W(G') = P[(X(\omega), \omega) \in G'] = P(S) = 1$, ou seja a medida $P_W$ tem suporte em $G'$.
