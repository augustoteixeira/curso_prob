...
\begin{proof}
  ...
  Introduzimos também em $\Omega \times E$ as projeções canônicas $X_\Omega$ e $X_E$.
  Utilizamos agora o Teorema~\ref{t:prob_cond_reg_F} para obter um núcleo $K^\star$ de $\Omega \times E$ em $\Omega$ que dá a probabilidade condicional regular de $X_\Omega$ (com respeito a $P^\star$) dada $\sigma(X_E)$, ou seja
  \begin{equation}
    K^\star(\cdot, A) = E^\star \big(\1_{[X_\Omega \in A]} | \sigma(X_E) \big) \text{ $P^\star$-q.c.}
  \end{equation}
  Mas como $K^\star(\cdot, A)$ é $\sigma(X_E)$-mensurável, podemos usar o Lema~\ref{l:f_g_circ_X} para obter uma $g_A: E \to \Omega \times E$ tal que
  \begin{equation}
    K^\star\big( (\omega, x), A \big) = g_A (X_E(\omega, x)) = g_A(x).
  \end{equation}
  ...
  \begin{equation}
    \begin{split}
      ...
      & \overset{\text{$P^\star$-q.c.}}= E^\star \big(\1_{[X_\Omega \in A]} | \sigma(X_E) \big) \circ X^\star.
    \end{split}
  \end{equation}
  Agora somente resta nos ver que essa é uma versão de $E(\1_A|\sigma(X))$, que segue da conta
  \begin{equation}
    \begin{split}
      E\big( E^\star(& \1_{[X_\Omega \in A]} | \sigma(X_E)) \circ X^\star \1_{[X \in B]}\big)\\
      & = E^\star \big( E^\star(\1_{[X_\Omega \in A]} | \sigma(X_E)) \1_{[X_E \in B]}\big)\\
      & = E^\star ( \1_{[X_\Omega \in A, pi_E \in B]} ) = E(\1_A \1_{[X \in B]}),
    \end{split}
  \end{equation}
  como gostaríamos.
\end{proof}
...
\begin{theorem}
  ...
  Se $P$ é uma probabilidade no espaço produto $(\Omega \times E, \mathcal{F} \otimes \mathcal{A})$ e denotamos por $P_X = P \circ X_1$ a primeira distribuição marginal de $P$, então existe um núcleo de transição $K: \Omega \times \mathcal{A} \to [0,1]$ satisfazendo
  \begin{equation}
    P = P_X \star K,
  \end{equation}
  ...
  \begin{equation}
    ...
    P(A \times B) = \int_A K(\omega, B) P_X (\d \omega) \text{ para todo $A \in \mathcal{F}$, $B \in \mathcal{A}$}.
  \end{equation}
  Nesse caso denotamos $K(\omega, B)$ por $P[X_2 \in B | X_1 = \omega]$.
\end{theorem}

\begin{proof}
  Como de costume, primeiramente resolveremos o caso $(E, \mathcal{A}) = (\mathbb{R}, \mathcal{B}(\mathbb{R}))$.
  Se assumimos a validade do teorema nesse caso, podemos usar a função bi-mensurável $\phi: E \to B \in \mathcal{B}(\mathbb{R})$ para concluir o caso geral.

  Lembrando que estamos no espaço $(\mathbb{R} \times \Omega, \mathcal{B}(\mathbb{R}) \otimes \mathcal{F}, P)$ denotamos a distribuição marginal $X_1 \circ P$ por $P_X$.
  Para cada $q \in \mathbb{Q}$, definimos $P^q_X : \mathcal{F} \to [0,1]$ por
  \begin{equation}
    P^q_X (A) = P\big( (-\infty, q] \times A \big).
  \end{equation}
  Observando que $P^q_X$ é absolutamente contínua com respeito a $P_X$, podemos definir
  \begin{equation}
    F(\omega, q) = \frac{\d P^q_X}{\d P_X}(\omega).
  \end{equation}
  Observamos as seguintes propriedades de $F$:
  \begin{enumerate}[\quad a)]
  \item para cada $q \in \mathbb{Q}$, $F(\cdot, q) \in [0,1]$, $P_X$-quase certamente, pois $P^q_X(A) \leq P_X(A)$ para todo $A \in \mathcal{F}$,
  \item para $q < q' \in \mathbb{Q}$, $F(\cdot, q) \leq F(\cdot, q')$, $P_X$-quase certamente, pois $P^q_X(A) \leq P^{q'}_X(A)$ para todo $A \in \mathcal{F}$ e
  \item $F(\cdot, n) \to 1$ (analogamente $F(\cdot, -n) \to 0$) quando $n$ tende a infinito, $P_X$-quase certamente. Para ver isso, note que a sequência de variáveis aleatórias $F(\cdot, n)$ é quase certamente monótona não decrescente, logo converge $P_X$-q.t.p., sendo limitada, converge em $\mathcal{L}^1$ e como sua integral em $P_X$ converge para um, $F(\cdot, n) \to 1$, quase certamente (analogamente para $F(\cdot, -n)$).
  \end{enumerate}
  Existe pois um conjunto $\Omega' \in \mathcal{F}$ com $P_X(\Omega') = 1$ no qual as três hipóteses acima são satisfeitas.
  Definimos $\hat{F}(\omega, q)$ como sendo igual a $F(\omega, q)$ em $\Omega'$ e como uma $F_0(q)$ fixa caso contrário (que claramente será mensurável).
  Finalmente podemos definir $\tilde{F}(\omega, x) = \inf_{q \in \mathbb{Q}; q \downarrow x} \hat{F}(\omega, q)$, que satisfaz as hipóteses do Teorema~\ref{t:existe_prob_R}.
  Logo, existe para cada $\omega \in \Omega$ uma medida $K(\omega, \cdot)$ em $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ satisfazendo $K(\omega,(-\infty, q]) = F(\omega, q)$ $P_X$-quase certamente.
  ...
  \begin{equation}
    \int_A K(\omega, B) P_X(\d \omega) = \int_A F(\omega, q) P_X (\d \omega) = P^q_X(A) = P(A \times B).
  \end{equation}
  Como a classe $B$ é um $\pi$-sistema gerando $\mathcal{B}(\mathbb{R})$ terminamos a prova do teorema.
\end{proof}
...
\begin{corollary}
  ...
  \end{equation}
  Também denotamos esse núcleo como $K(x, F) = P[F | X = x]$.
  Além disso vale o que chamamos de Princípio da Substituição:
  \begin{equation}
    K(x, [X = x]) = 1, \text{ $X \circ P$-quase certamente}.
  \end{equation}
  Que pode ser dito de maneira estranha: $P[X = x|X = x] = 1$, quase certamente.
\end{corollary}

\begin{proof}
  Defina o elemento aleatório $W: \Omega \to E \times \Omega$, dado por $W(\omega) = (X(\omega), \omega)$, que nada mais é que o gráfico de $X$ representado horizontalmente.
  ...
  Logo, para provar \eqref{e:reg_cond_prob_givenX}, basta ver que se $B \in \sigma(X)$, temos $B = [X \in A]$ para algum $A \in \mathcal{A}$, donde
  \begin{equation}
    \begin{split}
  ...
  Para mostrarmos o Princípio da Substituição, veja que
\end{proof}
