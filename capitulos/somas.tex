...
\todo{Melhorar a intuição da prova: 1) passo de redução para $E(X) < 0$, $a = 0$, $M < \infty$ q.c. 2) Falar sobre o desejo de escrever $M_{k + 1}$ em termos de $M_k'$, para enfatizar a ergodicidade. 3) mencionar que $E(\min(M'_k, -X_1)) \leq 0$ indica que $M'_k$ deve atuar para baixar a esperança de $-X_1$ a valores negativos 4) lei $\{0, 1\}$ impede que $M'$ atue nesse sentido.}
