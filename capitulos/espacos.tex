...
\subsection{Definição e propriedades básicas da esperança}
...
\item (Integral em um conjunto de medida zero) se $A \in \mathcal{F}$ tem $P(A) = 0$, então $\int_A f = 0$ para toda $f$ mensurável.
\item (Teorema de convergência dominada) se $(X_n)_{n\ge 1}$ for uma sequencia de variáveis  aleatórias  tal que para todo $\go$ temos $|X_n(\go)| \le Z(\go)$, onde $Z$ é integrável, e tal que $\lim_{n\to \infty} X_n(\go)=X(\go)$ q.c.
Então $X$ e integrável e
$$\lim_{n\to \infty} E[X_n]=E[X].$$
 \item (Troca soma/esperança) Se $(X_n)_{n\ge 1}$ é uma família de variáveis aleatórias tal que $\sum_{n\ge 1} E[|X_n|]<\infty$,
 então $Z := \sum_{n \ge 1} X_n$ é integrável e $E(Z) = \sum_{n \geq 1} E(X_n)$.
...
\item (Convergência Monótona) se $(X_n)_{n\ge 1}$ for uma sequencia de variáveis aleatórias  integráveis tais que para quase todos $\go$ a sequencia $(X_n(\go))_{n\ge 0}$ é crescente, então $X = \lim_{n \to \infty} X_n(\go)$ é integrável se $E[X_n]$ converge e nesse caso
  \begin{equation}
    \lim_{n \to \infty} E[X_n] = E[X].
  \end{equation}
...
\begin{proposition}
  ...
  \end{equation}
  Se $Y$ é uma variável aleatória e $f$ é diferenciável e tal que $E[|f'(Y)|] < \infty$ então
    \begin{equation}
    E[f(Y)] = \int_\bbR f'(y) [1 - F(y)] \d y.
  \end{equation}
\end{proposition}
...
\begin{proof}
  ...
  \end{equation}
    \begin{equation}
    \begin{split}
      E[f(Y)] & = - E \Big( \int_{-\infty}^Y f'(y)  \dd y \Big) = E \Big( \int_\bbR f'(y) \1_{[Y>y]} \d y \Big)\\
      & \overset{\text{Fubini}}= \int_\bbR f'(y) E(\1_{[Y>y]}) \d y = \int_\bbR f'(y) P[Y>y] \d y.
    \end{split}
  \end{equation}
\end{proof}
...
\begin{proposition}[Desigualdade de Jensen]
  ...
  \end{equation}
  Similarmente, se $\varphi: \bbR^d \to \bbR$ é convexa e ${\bf X}$ é um vector aleatório, sob hipóteses adequadas de integrabilidade temos
  \todo{pensar nas hipóteses}
  \begin{equation}
...
\begin{proof}
  Se, para alguma escolha de $a, b \in \mathbb{R}$, temos que $\varphi(x) \geq a x + b$ para todo $x \in \mathbb{R}$, então temos que $E(\varphi(X)) \geq a E(X) + b$ por linearidade e monotonicidade da esperança.
  O resultado segue ao tomaramos o supremo sob todas as funções afins (isso é, da forma $a x + b$) abaixo de $\varphi$.
  (O caso vectorial fica como exercício)
\end{proof}
