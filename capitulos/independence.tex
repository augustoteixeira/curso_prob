...
\begin{definition}
  Dizemos que uma coleção infinita de eventos $(A_n)_{n\ge 1}$
  é independente \index{independencia@independência!de eventos} se toda sub-coleção finita de tais eventos forem independentes.
\end{definition}
...
\begin{exercise}
  ...
  Mostre que este conjunto não é necessariamente uma $\sigma$-álgebra.
\end{exercise}
...
\begin{definition}
  Dado um espaço de probabilidade $(\Omega,P,\mathcal{F})$
Dizemos que as $\sigma$-álgebra $\mathcal{F}_1,\dots,\mathcal{F}_n\subset \mathcal{F}$
são independentes \index{independencia@independência!de sigma-algebras@de $\sigma$-álgebras}
se
\begin{equation}
 \forall A_1\in \mathcal{F}_1,\dots ,A_n \in \mathcal{F}_n, \ P(\cap_{i=1}^n A_i)=\prod_{i=1}^n P(A_i).
\end{equation}
...
Mostrar que as $\sigma$-álgebras $\mathcal{F}_1,\dots \mathcal{F}_n$ definidas for $\mathcal{F}_i:=\{\emptyset, A_i,A^{\cc}_i, \Omega\}$ são independentes se e só se
...
\begin{definition}
Sejam $X_1, \dots, X_k$ elementos aleatórios definidos respetivamente nos espaços $(E_i, \mathcal{A}_i)$ para $i=1,\dots,k$.
 ...
 \begin{equation}
 \forall A_1\in \mathcal{A}_1,\dots ,A_k \in \mathcal{A}_k, \ P([X_1\in A_1]\cap \dots \cap [X_k\in A_k])=\prod_{i=1}^k P[X_ i\in A_i].
\end{equation}
...
\begin{itemize}
 \item Consideramos $\Omega=\mathbb{R}^2$, e $P$ com densidade com respeito a Lebesgue $\rho_1(\omega_1)\rho_2(\omega_2)$.
Então as variáveis coordenadas $X_1$ e $X_2$ são independentes com distribuições respectivas $P_i(\dd x)=\rho_i(x)\dd x$.
 \item Se $\Omega_1$ e $\Omega_2$ são espaços enumeráveis e $P_1$ $P_2$ são probabilidades.
 Podemos definir $P$ a probabilidade em $\Omega=\Omega_1\times \Omega_2$ por
 $$p_{(\omega_1,\omega_2)}=P_1(\{\omega_1\})P_2(\{\omega_2\}).$$
 Com essa definição as variáveis $X_i: \Omega\to \Omega_i$ definidas por $X_i(\omega)=\omega_i$ são independentes.
...
\begin{exercise}
  ...
  \end{equation}
  \begin{enumerate}[\quad a)]
  \item Mostre que a distribuição conjunta $\mu_{(X,Y)}$ \'e
    absolutamente cont\'inua com relação à medida de Lebesgue em
    $\mathbb{R}^2$.
...
\begin{proof}
Começamos para verificar o resultado para  funções simples. Se $\alpha_1,\dots,\alpha_n$ e $\beta_1,\dots,\beta_m$ são os possíveis valores de $X$ e $Y$
temos
\begin{multline}
 E[XY]:=\sum_{i=1}^n \sum_{j=1}^m \alpha_i \beta_j P[ X=\alpha_i, Y=\beta_j]
 =  \sum_{i=1}^n \sum_{j=1}^m \alpha_i \beta_j P[ X=\alpha_i] P[Y=\beta_j]\\ = \left( \sum_{i=1}^n \alpha_i \beta_j P[ X=\alpha_i]\right)
 \left( \sum_{j=1}^m gb_j P[Y=\beta_j] \right)=E[X]E[Y],
\end{multline}
...
\begin{proposition}
Se  $\Omega:=\prod_{i=1}^n \Omega_i$ e $P:=\bigotimes_{i=1}^n P_i$ então os elementos aleatórios
$X_i$ definidos pelas projeções sob as coordenadas $X_i(\omega)=\omega_i$ são independentes
...
\begin{lemma}
\label{l:vt_conv}
Sejam $\mu$, $\nu$ duas medidas em $\Omega$ enumerável e $X:\  (\Omega,\mathcal{P}(\Omega))\to (E,\mathcal{A})$ um elemento aleatório
\begin{equation}
...
\begin{proof}
O segundo ponto segue do primeiro aplicado ao caso $\Omega=\mathbb{Z}^2$, $E=\mathbb{Z}$ e $X:\ (x,y) \mapsto (x+y)$.
...
$E_1,\dots,E_n$, podemos construir um espaço de probabilidade $(\Omega,\mathcal{F},P)$, tal que existem elementos aleatórios $X_i:\Omega\to E_i$,
...
\begin{equation}
  \Omega = \prod_{n=1}^{\infty} E_n := \big\{\omega=(\omega_n)_{n\in \mathbb{N}} \, : \, \forall n\ge 1, \   \omega_n \in E_n \big\}.
\end{equation}
...
\end{equation}
que é a menor $\sigma$-álgebra em $\Omega$ que deixa as applicações coordenadas mensúraveis.
Chamamos $\mathcal{F}$ de $\sigma$-álgebra canônica (o $\sigma$-álgebra produto), pode ser denotada informalmente como $\bigotimes_{n=1}^{\infty}\Omega_i$.
\end{definition}
...
\begin{definition}
Dada uma coleção de espaços de probabilidade $(E_n,\mathcal{A}_n,\mu_n)_{n\ge 1}$, e
sendo $(\Omega,\mathcal{F})$ o espaço produto associado. Chamamos de probabilidade produto dos $(\mu_n)_{n\ge 1}$ em $\Omega$ uma probabilidade $P$ que satisfaz
\begin{multline}
\forall n\in \mathbb{N}, \forall A_1\in \mathcal{A}_1,\dots, A_n\in \mathcal{A}_n, \\ P(\omega_1\in A_1,\dots ,\omega_n\in A_n)=\prod_{i=1}^n \mu_i(A_i).
\end{multline}
Isso e equivalente a dizer que as variáveis coordenadas $X_i(\omega)$ são independentes e identicamente distribuídas.
...
\begin{proof}
 Sabemos que $\mathcal{F}$ e gerada pelos eventos cilíndricos que formam um $\pi$-sistema.
...
\subsection{Construção de uma sequência de variáveis $\Ber(1/2)$ independentes}
 ...
 Consideramos $\Omega=[0,1]$ e $P=U_{[0,1]}$.
 Para $\omega\in[0,1]$, definimos $(X_n)_{n\ge 1}$ a sequencia de $\{0,1\}^{\mathbb{N}}$ tal que

 \begin{equation}\label{eq:deco}
 \omega:= \sum_{n=1} X_n(\omega)2^{-n}.
 \end{equation}
 ...
 \begin{proof}
Verificamos que para qualquer $q\in \mathbb{N}$ e$\gep=(\gep_1,\dots,\gep_q)\in \{0,1\}^q$,
  temos
...
Para mostrar \eqref{bassico} observamos que
  $$ \{ \omega \ | \ X_{1}=\gep_1, \dots, \ X_{q}=\gep_q \}= [x(q,\gep),x(q,\gep)+2^{-q}),$$
...
Seja $(\mu_n)_{n\ge 1}$ uma sequência de medida de probabilidade em $\mathbb{R}$ (com a $\sigma$ álgebra de Borel).
Queremos construir um espaço de probabilidade e uma sequência de variáveis independentes $X_n(\omega)$ tal que que para todo $n\in\mathbb{N}$
$(X_n)_*P=\mu_n$.


\medskip

Definimos $\Omega:=\{0,1\}^{\mathbb{N}}$ e $P$ sendo a medida definida na secção anterior para qual
as coordenadas $Y_n(\omega)$, $n\ge 1$  são Bernoulli independentes de parâmetro $1/2$.
Consideramos $m: \mathbb{N}\to \mathbb{N}^2$ a bijecção definida por
...
\end{lemma}
 ...
 \begin{proof}
Para identificar a distribuição de $Z_n$, observamos que por unicidade da probabilidade produto em $\{0,1\}^{\mathbb{N}}$, $Z_n$ tem que ter a mesma distribuição que
$\omega$ em \eqref{eq:deco}.
...
\begin{exercise}
  ...
  A coleção $(X_i)_{i \geq 0}$ é distribuída de maneira \iid e uniforme nas figurinhas.
  \begin{enumerate}[\quad a)]
  \item Para $j = 1, \dots, N$, seja $T_j$ o tempo passado até a aquisição da $j$-ésima nova figurinha, i.e.
    \begin{equation}
...
\begin{exercise}
  ...
  \end{equation}
  Supondo que $P[X_i=x]=0$ para todo $x\in\mathbb{R}$, encontre $E(R)$.
\end{exercise}
...
\begin{theorem}[Lei $\{0,1\}$ de Kolmogorov]
  ...
  Se $\Omega = E^{\mathbb{N}}$, onde $E$ é um espaço canônico, for provido de uma lei produto $P = \otimes_{i=1}^\infty P_i$, então todo evento caudal tem probabilidade $0$ ou $1$ sob $P$.
\end{theorem}
...
\begin{exercise}
  Dizemos que uma probabilidade $P$ no espaço produto $\Omega = E^{\mathbb{N}}$ (com a $\sigma$-álgebra canônica) é fortemente misturadora se, para todo $k \geq 1$, temos
  \begin{equation}
