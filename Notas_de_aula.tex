...
A probabilidade moderna se baseia fortemente na Teoria da Medida e supomos durante esse curso que o leitor esteja bem familiarizado com conceitos tais como: Medida de Lebesgue, extensões de medida, teoremas de convergência.
Iremos brevemente agora justificar a escolha da Teoria da Medida para o estudo de probabilidade.

Inicialmente, frequentemente foram estudados fenômenos probabilísticos para os quais apenas um número finito de resultados era possível, como por exemplo ao se jogar um dado de seis lados.
Em tais casos é desnecessário o uso de ferramentas sofisticadas pra modelar tais situações.
Por exemplo, podemos simplesmente dizer que as probabilidade $p_i$ de obter o lado $i$ são todas iguais a $1/6$.
...
Mas como podemos continuar nossa modelagem se nem ao menos podemos dizer qual é a probabilidade de chover um determinado volume esse ano, por exemplo $\pi/19$?
...
Gostaríamos sim de responder perguntas do tipo: qual é a probabilidade de que chova mais do que $37$?
...
Se nos inspiramos no exemplo do dado, poderíamos também nos interessar pela probabilidade de eventos, tais como: o dado lado sorteado é ímpar ($p_1 + p_3 + p_5$) ou o lado serteado seja dois ($p_2$).
E percebemos rapidamente que para eventos disjuntos a probabilidade de sua união é a soma de suas probabilidades (no caso acima $p_1 + p_2 + p_3 + p_5$) e se isso certamente nos lembra dos conceitos básicos de Teoria da Medida.
Vamos agora rever a discussão acima com mais calma, sob a ótica dessa teoria.
...
Inspirados na Teoria da Medida, podemos introduzir a seguinte definição.
\begin{definition}
...
\begin{example} Probabilidades nos espaços do Exemplo~\ref{x:espacos_amostrais}
  ...
  \item $P_3 = \delta_{f \equiv 0}$, que atribui o valor um se o evento contém a função identicamente nula e zero caso contrário.
  \end{enumerate}
\end{example}
...
\begin{example} Consideramos os casos
  ...
  \end{enumerate}
\end{example}
Seguindo a motivação do Exemplo~\ref{x:espacos_amostrais} $c)$, podemos estar interessados apenas na variável aleatória $X:\Omega_3 \to \mathbb{R}$ dada por $X(f) = f(1)$.
