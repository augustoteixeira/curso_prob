...
Uma família $\mathcal{G} \subseteq \mathcal{P}(\Omega)$ é dita um anel de conjuntos \index{anel de conjuntos} se vale
\begin{enumerate}[\quad a)]
...
\begin{lemma}[Extensão por continuidade no vazio]
  ...
  \end{enumerate}
  Então existe uma medida $\widebar{P}: \sigma(\mathcal{G}) \to \mathbb{R}_+$ tal que $\widebar{P}(A) = P(A)$ para $A \in \mathcal{G}$.
\end{lemma}

Observe que não supusemos $P(\Omega) = 1$, mas no contexto de medidas infinitas, não é de se esperar que $B_i \downarrow \varnothing$ implique $\lim_i \mu(B_i) = 0$, como foi assumido acima (relembre a Proposição~\ref{p:prob_continua}).
Portanto escrevemos o enunciado com $P$ para lembrar que quase sempre utilizaremos esse lema para construir probabilidades.
...
\begin{proof}
  Iremos mostrar que a propriedade \eqref{e:aditiva_na_algebra} é válida para $P$, logo tome $A_1, A_2, \dots \in \mathcal{G}$ disjuntos e tais que $A = \cup_i A_i \in \mathcal{G}$.
...
\begin{proof}
  ...
  \begin{equation}
    \mathcal{S} = \big\{ A \times \mathbb{R} \times \dots; \text{ onde } l \geq 1 \text{ e } A \in \mathcal{B}(\mathbb{R}^l) \big\}.
  \end{equation}
  Claramente, $\mathcal{S}$ é uma álgebra.

  Se $B = A \times \mathbb{R} \times \dots \in \mathcal{S}$ com $A \in \mathcal{B}(\mathbb{R}^l)$ como acima, definimos
  \begin{equation}
    P(B) = P_l(A).
  \end{equation}
  ...
  Para tanto, tome uma sequência encaixada $B_1 \supseteq B_2 \supseteq \dots \in \mathcal{S}$ e, supondo que $P(B_n) \geq \delta > 0$ para todo $n \geq 1$, gostaríamos de mostrar que sua interseção não pode ser vazia.

  Como $B_n \in \mathcal{S}$, podemos escrever
  \begin{equation}
    B_n = A_n \times \mathbb{R} \times \dots, \text{ onde $A_n \in \mathcal{B}(\mathbb{R}^{l_n})$ e $n \geq 1$.}
  \end{equation}
  ...
  \begin{equation}
    C_n = C_n^* \times \mathbb{R} \times \dots, \text{ com $C_n^* \in \mathcal{B}(\mathbb{R}^{l_n})$}
  \end{equation}
  ...
  Isso pode ser feito observando que podemos escrever $D_n = D_n^* \times \mathbb{R} \times \mathbb{R} \times \dots$, onde $D_n^* \in \mathcal{B}(\mathbb{R}^{l_n})$ e
  \begin{equation}
...
Poderíamos na verdade substituir $\mathbb{R}$ por qualquer espaço métrico sequencialmente compacto e $\sigma$-compacto.
Contudo, não apresentamos essa versão mais geral aqui pois muito em breve vamos utilizar o caso da reta para mostrar uma versão bem mais geral do Teorema de Kolmogorov.
...
Talvez o mais importante exemplo do uso deste teorema seja o seguinte.
...
\todosec{Tópico: Contando triângulos}{números de triângulos em grafos (olhar alon spencer)...}
